...
public class StrippableLilacLog extends LogBlock {
    private Random rand = new Random();
    private double sapProbability = 0.0625;
    private BlockState oozingState = null;
    private BlockState strippedState = null;
    public StrippableLilacLog(MaterialColor colorIn, Properties properties) {
        ...
        if(player.getHeldItem(handIn) != ItemStack.EMPTY) {
            ...
            if(item == Items.DIAMOND_AXE || item == Items.IRON_AXE || item == Items.GOLDEN_AXE || item == Items.WOODEN_AXE
                ...
                if (state.get(AXIS) == Direction.Axis.X){
                    ...
                    oozingState = RegistryHandler.OOZING_LILAC_BLOSSOM_LOG.get().getDefaultState().with(AXIS, Direction.Axis.X);
                    strippedState = RegistryHandler.STRIPPED_LILAC_BLOSSOM_LOG.get().getDefaultState().with(AXIS, Direction.Axis.X);
                } else if (state.get(AXIS) == Direction.Axis.Z){
                    System.out.println("Z axis");
                    oozingState =  RegistryHandler.OOZING_LILAC_BLOSSOM_LOG.get().getDefaultState().with(AXIS, Direction.Axis.Z);
                    strippedState = RegistryHandler.STRIPPED_LILAC_BLOSSOM_LOG.get().getDefaultState().with(AXIS, Direction.Axis.Z);
                } else {
                    System.out.println("Y axis");
                    oozingState = RegistryHandler.OOZING_LILAC_BLOSSOM_LOG.get().getDefaultState();
                    strippedState = RegistryHandler.STRIPPED_LILAC_BLOSSOM_LOG.get().getDefaultState();
                }
